	...
	*/
	...
	app.init = function (container) {
		...
		window.addEventListener('click', function(event) {
  			...
  			if (intersects.length > 0) {
    			var feature = intersects[0].object.userData;
   			 	alert("Building: " + feature.name + "\nHeight: " + feature.height + " m");
  			}
		});
		...
		app.renderer.domElement.addEventListener('mousemove', app.eventListener.mousemove);
		app.renderer.domElement.addEventListener('mouseout', app.eventListener.mouseout);

		gui.init();

		// Add hover tooltip methods to the app object
		app.hoverTooltip = null;
		app.hoverTimeout = null;

		app.showHoverTooltip = function (event, properties, layer) {
			...
			if (!app.hoverTooltip) {
				app.hoverTooltip = document.createElement('div');
				app.hoverTooltip.className = 'hover-tooltip';
				app.hoverTooltip.style.cssText = `
					position: fixed;
				...
				`;
				document.body.appendChild(app.hoverTooltip);
			}
			
			// Build tooltip content with building attributes
			var tooltipContent = '<div style="font-weight: bold; margin-bottom: 4px;">Building Information</div>';
			
			// Add common building attributes
			if (properties.fid !== undefined) {
				tooltipContent += '<div><strong>FID:</strong> ' + properties.fid + '</div>';
			}
			if (properties.id !== undefined) {
				tooltipContent += '<div><strong>ID:</strong> ' + properties.id + '</div>';
			}
			if (properties.height !== undefined) {
				tooltipContent += '<div><strong>Height:</strong> ' + properties.height + ' m</div>';
			}
			if (properties.building !== undefined) {
				tooltipContent += '<div><strong>Building:</strong> ' + properties.building + '</div>';
			}
			
			// Add any other properties (excluding internal ones)
			for (var key in properties) {
				if (key !== 'fid' && key !== 'id' && key !== 'height' && key !== 'building' && 
					properties.hasOwnProperty(key) && 
					typeof properties[key] !== 'object' && 
					properties[key] !== null) {
					tooltipContent += '<div><strong>' + key + ':</strong> ' + properties[key] + '</div>';
				}
			}
			
			app.hoverTooltip.innerHTML = tooltipContent;
			
			// Position tooltip near cursor
			var offsetX = 15;
			var offsetY = 15;
			
			app.hoverTooltip.style.left = (event.clientX + offsetX) + 'px';
			app.hoverTooltip.style.top = (event.clientY + offsetY) + 'px';
			app.hoverTooltip.style.display = 'block';
		};

		app.hideHoverTooltip = function () {
			if (app.hoverTooltip) {
				// Use timeout to prevent flickering when moving between buildings
				app.hoverTimeout = setTimeout(function() {
					if (app.hoverTooltip) {
						app.hoverTooltip.style.display = 'none';
					}
				}, 100);
			}
		};
	...
	app.eventListener = {
        ...
        for (var i = 0, l = objs.length; i < l; i++) {
            ...
            if (!layer || layer.type !== Q3D.LayerType.Polygon) continue;
            
            // Get the actual feature object
            var featureObj = o;
            while (featureObj && featureObj.userData.featureIdx === undefined) {
                featureObj = featureObj.parent;
            }
            
            if (featureObj && featureObj.userData.properties) {
                app.showHoverTooltip(e, featureObj.userData.properties, layer);
                return;
            }
        }
        
        // No building hovered, hide tooltip
